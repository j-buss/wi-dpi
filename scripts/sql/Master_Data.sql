...
SELECT
  ...
  MAX(real_2016.contract_high_degree_cd) as max_contract_high_degree_cd,
  MAX(real_2016.contract_local_experience) as max_contract_local_experience,
  MAX(real_2016.contract_total_experience) as max_contract_total_experience,
...
FROM
  ...
  INNER JOIN `wi-dpi-010.Merged.Educator_Position_Classification_by_Year` pos_class
  ON (real_2016.research_id = pos_class.key and pos_class.year = "2016")
  INNER JOIN `wi-dpi-010.Merged.Educator_Role_Summary_by_Year` role_summary
  ON (real_2016.research_id = role_summary.key and role_summary.year = "2016")
GROUP BY
  real_2016.research_id,
  pos_class.teacher_cnt,
  pos_class.admin_cnt,
